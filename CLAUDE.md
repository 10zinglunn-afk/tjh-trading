...
| `api_server.py` | FastAPI wrapper around the harness. `/api/run` (single-ticker via `export_results.build`) + the **Engine Room** routes (2026-07-06): `/api/universe` (per-ticker data provenance), `/api/engine/scan` (live `scan.scan_universe`), `/api/engine/momentum` (live `xsect` pipeline), `/api/alpaca/status` (paper account, graceful when keys absent). Deps: `requirements-api.txt` (now includes `alpaca-py`). |
