...
| `xsect.py` | **Cross-sectional panel engine** (Thesis 001). Monthly 12-1 momentum rank across the stock universe (ETFs excluded), top-10 equal weight, net of costs on full turnover, `weights.shift(1)` no-lookahead. Canonical spec only (n_trials=1 — no grid). Judged vs **EW-universe** (the honest bar vs survivorship), random picks, and SPY, plus a per-year split. First run 2026-07-03: **SURVIVES** (pending J/H sign-off). |
